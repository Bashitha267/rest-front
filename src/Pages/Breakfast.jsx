...
useEffect(()=>{
    const fetchdata=async()=>{
        if(loading){
            ...
            setloading(true);
            try{
                const response= await axios.get("http://localhost:3000/getbreakfast")
            ...
            <div className="md:grid md:grid-cols-3 md:w-3/4 md:place-items-center md:gap-1 md:mt-5">
              {
                breakfastdata.map((item,index)=>(
                    <div key={breakfastdata._id||index} className="flex flex-col w-full transform transition-transform duration-300 hover:scale-105 mb-6">
                    <div>
                      <img
                        src={
                          item.img
